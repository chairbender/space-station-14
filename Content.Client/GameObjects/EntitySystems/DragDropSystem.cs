...
#pragma warning restore 649
        ...
        private GameScreenBase _gameScreenBase;
        ...
        public override void Initialize()
        {
            ...
            // only subscribe to drag and drop when in game
            _stateManager.OnStateChanged += OnStateChanged;
            _dropTargetInRangeShader = _prototypeManager.Index<ShaderPrototype>(ShaderDropTargetInRange).Instance();
            _dropTargetOutOfRangeShader = _prototypeManager.Index<ShaderPrototype>(ShaderDropTargetOutOfRange).Instance();
            //TODO: use the new way for this
            _interactionSystem = EntitySystemManager.GetEntitySystem<SharedInteractionSystem>();
        }

        private void OnStateChanged(StateChangedEventArgs obj)
        {
            var inputSystem = EntitySystemManager.GetEntitySystem<InputSystem>();
            if (obj.NewState is GameScreen gameScreenBase)
            {
                _gameScreenBase = gameScreenBase;
                // needs to fire on mouseup and mousedown so we can detect a drag / drop
                inputSystem.BindMap.BindFunction(EngineKeyFunctions.Use,
                    new PointerInputCmdHandler(OnUse, false));
            }
            else
            {
                Unbind(inputSystem);
        ...
        private void Unbind(InputSystem inputManager)
        {
            _gameScreenBase = null;
            inputManager.BindMap.UnbindFunction(EngineKeyFunctions.Use);
        }

        public override void Shutdown()
        {
            CancelDrag();
            if (EntitySystemManager.TryGetEntitySystem(out InputSystem inputSystem))
            {
               Unbind(inputSystem);
            }
        ...
        {
            if (_gameScreenBase == null) return false;
        ...
        private bool OnUseMouseUp(PointerInputCmdHandler.PointerInputCmdArgs args)
        {
            ...
            if (_interactionSystem.InRangeUnobstructed(dragger.Transform.MapPosition,
                args.Coordinates.ToMapPos(_mapManager), ignoredEnt: dragger) == false)
            {
                {
                    return false;
                }
            ...
            var entities = _gameScreenBase.GetEntitiesUnderPosition(args.Coordinates);
            foreach (var entity in entities)
            {
                if (entity.TryGetComponent<IClientDraggable>(out var draggable))
                {
        ...
        private bool OnUseMouseDown(PointerInputCmdHandler.PointerInputCmdArgs args)
        {
            ...
            if (_interactionSystem.InRangeUnobstructed(_dragger.Transform.MapPosition,
                args.Coordinates.ToMapPos(_mapManager), ignoredEnt: _dragger) == false)
            {
                {
                    CancelDrag();
                    return false;
        ...
        private void HighlightTargets()
        {
            ...
            {
                var inRangeSprite = pvsEntity.GetComponent<SpriteComponent>();
                // can't highlight if there's no sprite or it's not visible
                if (inRangeSprite == null || inRangeSprite.Visible == false) continue;
                ...
                if (_draggable.ClientCanDropOn(new CanDropEventArgs(_dragger, _draggedEntity, pvsEntity)))
                {
                    ...
                    var inRange = _interactionSystem.InRangeUnobstructed(_dragger.Transform.MapPosition,
                        pvsEntity.Transform.WorldPosition, ignoredEnt: _dragger);
        ...
        public override void Update(float frameTime)
        {
            ...
            else if (_state == DragState.DRAGGING)
            {
                ...
                //TODO: use the new way for this
                if (_interactionSystem.InRangeUnobstructed(_dragger.Transform.MapPosition,
                    _draggedEntity.Transform.WorldPosition, ignoredEnt: _dragger) == false)
                {
                    CancelDrag();
                    return;
                }
